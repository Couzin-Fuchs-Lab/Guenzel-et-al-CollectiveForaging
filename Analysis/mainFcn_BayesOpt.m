...
SET.OptimizeParameters = true;

% Paths
SET.OptimizeParametersFile = 'ModelParameters.mat';
SET.OutputFile = 'ModelOutput.mat';
...
if SET.OptimizeParameters
    ...
    % Save results
    save(SET.OptimizeParametersFile, 'ModelParameters', 'SET')
end%if optimize
...
for iCond = 1:length(SET.ConditionNames.Patch)
    ...
    for iGrp = 1:length(SET.ConditionNames.Group)
        ...
        ModelOutput.(SET.ConditionNames.Patch{iCond}).(SET.ConditionNames.Group{iGrp}).id = [];
        ModelOutput.(SET.ConditionNames.Patch{iCond}).(SET.ConditionNames.Group{iGrp}).both = [];
        ModelOutput.(SET.ConditionNames.Patch{iCond}).(SET.ConditionNames.Group{iGrp}).ind = [];
        ModelOutput.(SET.ConditionNames.Patch{iCond}).(SET.ConditionNames.Group{iGrp}).soc = [];
...
% Save results
save(SET.OutputFile , 'ModelData', 'ModelOutput', 'ModelParameters', 'SET')
