...
classdef SubFcn
    ...
    methods(Static)
        ...
        function out = VisitHistory(TC_A, TC_B)
            ...
            A_diff = diff(TC_A);
            B_diff = diff(TC_B);
            join_A = find(A_diff>0);
            join_B = find(B_diff>0);
            ...
            if isempty(JoinEvents)
                out = zeros(length(TC_A),1);
            else
                % Preallocation
                out = nan(length(TC_A),1);
                Tbl = zeros(size(JoinEvents,1),1);
                for iEvent = 1:size(JoinEvents,1)
                    ...
                    out(JoinEvents(iEvent,1)) = Tbl(iEvent,1);
                end%iEvent
        ...
        function InstaFreq = GetInstaFreq(Events, Dur, SamplingRate)
            ...
            % Dur:           Duration in number of frames of the original signal
            ...
            % Iterate over events
            for iEvent = 2:length(Events)
                InstaFreq(Events(iEvent-1):Events(iEvent)-1) = ...
        ...
        function Switching_t1 = GetSwitching_t1(TC_A, TC_B)
            ...
            [~,idx] = sort(JoinEvents(:,1));
            JoinEvents = JoinEvents(idx,:);
            % Iterate over joining events
            if size(JoinEvents,1)>=2
                for iEvent = 2:size(JoinEvents,1)
                    % ++
                    if JoinEvents(iEvent-1,2)==1 && JoinEvents(iEvent,2)==1
                        Switching_t1(1,1) = Switching_t1(1,1)+1;
                        % +-
                    elseif JoinEvents(iEvent-1,2)==1 && JoinEvents(iEvent,2)==-1
                        Switching_t1(1,2) = Switching_t1(1,1)+1;
                        % -+
                    elseif JoinEvents(iEvent-1,2)==-1 && JoinEvents(iEvent,2)==1
                        Switching_t1(2,1) = Switching_t1(2,1)+1;
                        % --
                    elseif JoinEvents(iEvent-1,2)==-1 && JoinEvents(iEvent,2)==-1
        ...
        function Switching_t2 = GetSwitching_t2(TC_A, TC_B)
            ...
            JoinEvents = JoinEvents(idx,:);
            % Iterate over joining events
            if size(JoinEvents,1)>=3
                for iEvent = 3:size(JoinEvents,1)
                    if sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [1 1 1]) == 3
                        Switching_t2(1,1) = Switching_t2(1,1)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [1 1 -1]) == 3
                        Switching_t2(1,2) = Switching_t2(1,2)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [1 -1 1]) == 3
                        Switching_t2(2,1) = Switching_t2(2,1)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [1 -1 -1]) == 3
                        Switching_t2(2,2) = Switching_t2(2,2)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [-1 1 1]) == 3
                        Switching_t2(3,1) = Switching_t2(3,1)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [-1 1 -1]) == 3
                        Switching_t2(3,2) = Switching_t2(3,2)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [-1 -1 1]) == 3
                        Switching_t2(4,1) = Switching_t2(4,1)+1;
                    elseif sum([JoinEvents(iEvent-2,2), JoinEvents(iEvent-1,2), JoinEvents(iEvent,2)] == [-1 -1 -1]) == 3
        ...
        function violinplot_advanced(ViolinData, pos, width, properties)
            ...
            properties = SubFcn.FillEmptyPropertyValues_violin(properties);
            % Exclude NaNs
        ...
        function lpf = tc_lowpass(x, tau)
            ...
            for t = 2:length(x)
                if abs(x(t))>0
                    ...
                    currTau = tau(2); % forget
                end
                % Calculate the change in signal
                delta = 1-exp(1)^(-(x(t)-lpf(t-1))/currTau);
                % Apply change
                lpf(t) = lpf(t-1)+delta;
            end%t
        end%FCN:tc_lowpass
